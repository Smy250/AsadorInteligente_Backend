...
class RegistroDePagos(Base):
    ...
    id_metodo_pago: Mapped[int] = mapped_column(
        Integer, ForeignKey("metodos_pago.id"), nullable=False)
    ...
    pago: Mapped[list['DetallePago']] = relationship(
        back_populates="detalles_pago")  # type: ignore
    metodo_pago: Mapped['MetodoPago'] = relationship(
        back_populates="registros_de_pagos")  # type: ignore
